...
namespace DigitalSigning.Core.Repositories
{
    ...
    public class TransactionRepository : MongoRepository<Transaction>, ITransactionRepository
    {
        ...
        public async Task<IReadOnlyCollection<Transaction>> GetByStatusAndAgeAsync(
            TransactionStatus status, DateTime olderThan)
        {
            var filter = Builders<Transaction>.Filter.And(
                Builders<Transaction>.Filter.Eq(t => t.CurrentStatus, status),
                Builders<Transaction>.Filter.Lt(t => t.UpdatedAt, olderThan));
            var cursor = await _collection.FindAsync(filter);
            return await cursor.ToListAsync();
        }

        public async Task<IReadOnlyCollection<Transaction>> GetExpiredWaitingAsync(DateTime now)
        {
            var filter = Builders<Transaction>.Filter.And(
                Builders<Transaction>.Filter.Eq(t => t.CurrentStatus, TransactionStatus.WaitingUserConfirm),
                Builders<Transaction>.Filter.Lt(t => t.WaitingExpireAt, now));
            var cursor = await _collection.FindAsync(filter);
            return await cursor.ToListAsync();
        }
    }
