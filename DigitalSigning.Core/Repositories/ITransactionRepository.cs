...
namespace DigitalSigning.Core.Repositories
{
    ...
    public interface ITransactionRepository : IMongoRepository<Transaction>
    {
        ...
        /// <summary>Get transactions by status and older than threshold (for recovery).</summary>
        Task<IReadOnlyCollection<Transaction>> GetByStatusAndAgeAsync(TransactionStatus status, DateTime olderThan);

        /// <summary>Get transactions stuck in WaitingUserConfirm that have expired.</summary>
        Task<IReadOnlyCollection<Transaction>> GetExpiredWaitingAsync(DateTime now);
    }
}