...
// Register consumer — listens to the Upload queue
builder.Services.AddSingleton<IMessageConsumer>(sp =>
    new MessageConsumer(
        ...
        RabbitMqTopologyInitializer.UploadQueue));

// Register the worker
builder.Services.AddHostedService<DigitalSigning.Workers.Upload.Worker>();
