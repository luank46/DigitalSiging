...
// This registers MongoDB, repositories, services, GridFs, FileDownloader, etc.
...
// Register consumer — listens to the FilePrepare queue
builder.Services.AddSingleton<IMessageConsumer>(sp =>
    new MessageConsumer(
        ...
        RabbitMqTopologyInitializer.FilePrepareQueue));

// Register the worker
builder.Services.AddHostedService<DigitalSigning.Workers.FilePrepare.Worker>();
